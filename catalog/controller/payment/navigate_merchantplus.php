...
class ControllerPaymentNavigateMerchantplus extends Controller {
	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/navigate_merchantplus.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/navigate_merchantplus.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/navigate_merchantplus.tpl', $data);
		}
	}

	public function send() {
		// if ($this->config->get('navigate_merchantplus_server') == 'live') {
		// 	$url = 'https://secure.authorize.net/gateway/transact.dll';
		// } elseif ($this->config->get('navigate_merchantplus_server') == 'test') {
		...
		// Transaction Info
		$data['x_method']             = 'CC';
		$data['x_type']               = ($this->config->get('navigate_merchantplus_method') == 'capture') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		...
		if ($this->config->get('navigate_merchantplus_mode') == 'test') {
		...
		if (curl_error($curl)) {
			...
			if ($response_info[1] == '1') {
				...
				if (!$this->config->get('navigate_merchantplus_hash') || (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('navigate_merchantplus_hash') . $this->config->get('navigate_merchantplus_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false))))) {
					$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('navigate_merchantplus_order_status_id'), $message, false);
				} else {
			...
			$this->log->write('Navigate CURL ERROR: Empty Gateway Response');
		}
