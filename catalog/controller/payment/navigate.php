...
class ControllerPaymentNaviGate extends Controller {
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/navigate.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/navigate.tpl';
		} else {
			$this->template = 'default/template/payment/navigate.tpl';
		}	
	...
	public function send() {
		if ($this->config->get('navigate_server') == 'live') {
			$url = 'https://gateway.merchantplus.com/cgi-bin/PAWebClient.cgi';
		} elseif ($this->config->get('navigate_server') == 'test') {
			$url = 'https://gateway.merchantplus.com/cgi-bin/PAWebClient.cgi';		
		}	
		...
		$data['x_login'] = $this->config->get('navigate_login');
		$data['x_tran_key'] = $this->config->get('navigate_key');
		$data['x_version'] = '3.1';
		...
		$data['x_method'] = 'CC';
		$data['x_type'] = ($this->config->get('navigate_method') == 'capture') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		...
		if ($this->config->get('navigate_mode') == 'test') {
		...
		if (curl_error($curl)) {
			...
			$this->log->write('NAVIGATE CURL ERROR: ' . curl_errno($curl) . '::' . curl_error($curl));	
		} elseif ($response) {
			$i = 1;
			...
			if ($response_info[1] == '1') {
				if (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('navigate_hash') . $this->config->get('navigate_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false)))) {
					...
					$this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('navigate_order_status_id'), $message, false);				
				}
